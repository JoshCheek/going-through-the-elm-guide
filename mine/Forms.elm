...
import Html.Attributes exposing (type_, style, value, placeholder, autofocus)
...
update msg formData =
    case msg of
        ...
        Submit ->
            if invalidAge formData.age then
                { formData
                    | status = Error
                    , messages = [ "Age must be a whole number of years" ]
                }
            else if String.length formData.password < minPasswordLength then
                { formData
            ...
            else if formData.password /= formData.passwordConfirmation then
                { formData
                    | password = blankForm.password
                    , passwordConfirmation = blankForm.passwordConfirmation
                    , status = Error
