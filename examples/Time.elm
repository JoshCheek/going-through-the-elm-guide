...
view clock =
  case clock of
    ...
    Seconds seconds ->
      let
        secondHand = seconds % 60
        minuteHand = (seconds // 60) % 60
        hourHand   = (seconds // 60 // 60) % 12
      in
        clockDisplay hourHand minuteHand secondHand


(@=) fn attr = fn (toString attr)
infixr 1 @=
...
clockDisplay hours minutes seconds =
  let
    ...
    hand ticksTaken ticksAvailable r width =
      let
        percentComplete = (toFloat ticksTaken) / (toFloat ticksAvailable)
        quarterTurn     = Basics.pi / 2
        fullTurn        = Basics.pi * 2
        angle           = fullTurn * percentComplete - quarterTurn
        distalX         = (cos angle) * r + noseX
        distalY         = (sin angle) * r + noseY
      in
        line
          [ x1 @= noseX,   y1 @= noseY
          , x2 @= distalX, y2 @= distalY
          , stroke "black"
